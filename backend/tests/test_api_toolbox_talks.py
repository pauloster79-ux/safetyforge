...
class TestCreateToolboxTalk:
    ...
    def test_create_toolbox_talk(self, client: TestClient, test_company):
        ...
        response = client.post(
            f"/api/v1/me/projects/{project_id}/toolbox-talks",
            json={
...
class TestListToolboxTalks:
    ...
    def test_list_toolbox_talks(self, client: TestClient, test_company):
        ...
        response = client.get(f"/api/v1/me/projects/{project_id}/toolbox-talks")
...
class TestGetToolboxTalk:
    ...
    def test_get_toolbox_talk(self, client: TestClient, test_company):
        ...
        response = client.get(
            f"/api/v1/me/projects/{project_id}/toolbox-talks/{talk_id}"
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_toolbox_talk_not_found(self, client: TestClient, test_company):
        ...
        project_id = _create_project(client)
        response = client.get(
            f"/api/v1/me/projects/{project_id}/toolbox-talks/talk_nonexistent123"
        )
...
class TestAddAttendee:
    ...
    def test_add_attendee(self, client: TestClient, test_company):
        ...
        response = client.post(
            f"/api/v1/me/projects/{project_id}/toolbox-talks/{talk_id}/attend",
            json={
...
class TestCompleteTalk:
    ...
    def test_complete_talk(self, client: TestClient, test_company):
        ...
        response = client.post(
            f"/api/v1/me/projects/{project_id}/toolbox-talks/{talk_id}/complete",
            json={
...
class TestBilingualGeneration:
    ...
    def test_bilingual_generation_prompt_structure(self, client: TestClient, test_company):
        """Verify generation includes the topic in the prompt."""
        project_id = _create_project(client)

        mock_response = MagicMock()
        mock_response.content = [
            MagicMock(
                text='{"en": {"topic_overview": "Confined space overview", "key_points": ["Always test atmosphere"], "discussion_questions": ["What PPE is needed?"], "safety_reminders": ["Never enter alone"], "osha_references": ["29 CFR 1926.1203"]}}'
            )
        ]
        ...
        with patch(
            ...
            # Verify the API was called with the topic
            assert mock_client_instance.messages.create.called
            call_args = mock_client_instance.messages.create.call_args
            user_message = call_args.kwargs["messages"][0]["content"]
            assert "Confined Space Entry" in user_message

        assert response.status_code == 201
        data = response.json()
        assert data["content_en"]["topic_overview"] == "Confined space overview"
...
class TestDeleteToolboxTalk:
    ...
    def test_delete_toolbox_talk(self, client: TestClient, test_company):
        ...
        list_resp = client.get(f"/api/v1/me/projects/{project_id}/toolbox-talks")
